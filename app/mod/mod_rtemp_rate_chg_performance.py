...
class ModRtempRateChgPerformance(DefaultItem):
    def preprocess(self):
        """
        It extracts data from a csv file and returns a list of lists
        :return: a tuple of two values. The first value is a boolean value indicating whether the function
        is executed successfully. The second value is the result of the function.
        """
        ...
        def extract_file(csv_file):
            ...
            try:
                ...
                return True, [csv_file.split('/')[-3], df.loc[min_index - 1, 'Amp Hours Discharge, AH'] - df.loc[max_index, 'Amp Hours Discharge, AH']]
        ...
        item_result = {
            "table": [["样本编号", "室温放电容量", "倍率充电容量", "倍率充电容量/室温放电容量", "是否合格"]]
        }

        for result in extract_results:
            item_result["table"].append([result[0], round(mrdc[result[0]], 2), round(result[1], 2), f"{result[1] / mrdc[result[0]]:.2%}", "合格" if result[1] / mrdc[result[0]] >= 0.8 else "不合格"])
